...
| MCP server registered but the agent cannot spawn it | `searchts` is inside `.venv\Scripts\` and is not on the agent's PATH. `searchts mcp install` warns: `Note: \`searchts\` must be on your PATH (e.g. installed with pipx). If it isn't, replace \`searchts\` with the full path to the executable.` | Register the absolute path, or install with `pipx install searchts` so the shim is global |
...
| `--human` "does nothing" - no browser opens | The headful fallback only fires when the failure reason is exactly `challenge` or starts with `http-403` (`searchts/unlocker.py`). `http-429`, `thin-<n>b`, DNS and TLS errors never trigger it. If patchright is also missing, the original error is re-raised and the patchright hint is swallowed | For `http-429` just wait. Otherwise check the per-backend reasons on stderr first |
