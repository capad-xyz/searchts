...
- [ ] **R1c** Auto-Hare on `opened` / `synchronize`. **Built 2026-09-09** (proving): `.github/workflows/hare.yml` + `scripts/hare_r1.py`. Secrets: Nous then OpenRouter then **OpenCode Zen**. Writer ≠ Hare (CI token is `github-actions[bot]`, not the orchestrator). Skip never holds. **Not R1b**. **Not F15**. Do not run the orchestrator as Hare.
  ...
  - **Failover (fixed list, not a router):** Nous `poolside/laguna-s-2.1` → OpenRouter `inclusionai/ling-3.0-flash-fin:free` → Zen `ling-3.0-flash-fin-free` (`https://opencode.ai/zen/v1`). Fail = 429 / 5xx / timeout / empty / no JSON. Exhausted → `searchts-r1-needed`. Intent from Check Runs in code. **No LLM in the router.** **Muse Spark contributor-free is not used:** OpenCode marks it free, but contributor endpoints train on prompts; public PR diffs stay off that. Muse is Responses API anyway (`/zen/v1/responses`), not Chat Completions. Revisit Muse only with a Responses client **and** a non-training SKU.
...
- [x] **R1b** GitHub App **identity only** — reviews show as `searchts-hare[bot]` (not `github-actions[bot]` / not `capad-xyz`). App: `searchts-hare`. One install: `capad-xyz/searchts`. Workflow mints an install token; if mint fails, fall back to `GITHUB_TOKEN`. Not a review product. Secrets: `HARE_APP_CLIENT_ID`, `HARE_APP_PRIVATE_KEY` (full PEM, `BEGIN` not `START`). Logo is the App avatar, not in git.
