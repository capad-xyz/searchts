...
- [ ] **R1c** Auto-Hare on `opened` / `synchronize`. **Built 2026-09-09** (proving): `.github/workflows/hare.yml` + `scripts/hare_r1.py`. Secrets: Nous then OpenRouter then **OpenCode Zen**. Writer ≠ Hare (CI token is `github-actions[bot]`, not the orchestrator). Skip never holds. **Not R1b**. **Not F15**. Do not run the orchestrator as Hare.
  ...
  - **Failover (fixed list, not a router):** Nous `poolside/laguna-s-2.1` → `stepfun/step-3.7-flash` → OpenRouter `poolside/laguna-s-2.1:free` → `qwen/qwen3.8-27b:free` → `nex-agi/nex-n2.5-pro:free` → Zen `ling-3.0-flash-fin-free` (`https://opencode.ai/zen/v1`). Fail = 429 / 5xx / timeout / empty / no JSON. Exhausted → `searchts-r1-needed`. **Hop cap 60s** (so the nag still posts); **job `timeout-minutes: 10`** is a backstop only (a kill can skip the nag). Intent from Check Runs in code. **No LLM in the router.** Skip `openrouter/free` (non-reproducible), Lyria (audio), GLM 5.2 (32k, no tools). **Muse Spark contributor-free is not used:** OpenCode marks it free, but contributor endpoints train on prompts; public PR diffs stay off that. Muse is Responses API anyway (`/zen/v1/responses`), not Chat Completions. Revisit Muse only with a Responses client **and** a non-training SKU.
...
- [ ] **F11** Windows: `pip install -e .` / `pipx install --force` still fails while `searchts mcp serve` holds the shim (OS error 32). RUNBOOK has the workaround (close those clients, or kill the PIDs, then install from a folder that is not an old checkout). **Not a fix.** **Do not** add `--single-instance`, a machine-wide mutex, or `searchts mcp stop`. A stop command would kill every host's server, including the one you are typing in, and the process holding the file still cannot replace itself. **Revisit:** a `doctor` line that prints the holder PIDs, only if this bite keeps happening. Do not auto-kill.
