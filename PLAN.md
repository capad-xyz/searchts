...
- [x] **R1** Implementer ≠ reviewer. Second agent is **🐇‍❄️ Hare** — a **cheap-scout**, not the orchestrator. Writer model ≠ Hare model. Grok holds merge; Grok-as-Hare only if every cheap path is dead (say so in the comment). Posts a GitHub PR comment (not only local chat) starting with `<!-- searchts-r1-review -->`, then Name / Purpose / Model / Effort + real-vs-skip table. Spec in [`AGENTS.md`](AGENTS.md). Revisit: whenever CodeRabbit is rate-limited.
...
| 2026-09-06 | **Hare ≠ orchestrator.** Cheap-scout reviews. Grok merge-only unless cheap path is dead. #132 Hare comment was Grok — that was the miss. |
