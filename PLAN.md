...
- [ ] **F11** Windows: `pip install -e .` still fails while `searchts.exe` is running. RUNBOOK has the workaround (close the MCP session, or `--force-reinstall`). **Not a fix.** **Do not** add `--single-instance` / a machine-wide mutex. **Revisit:** only if a shim can be replaced without a machine-wide lock.
