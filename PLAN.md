...
- [ ] **R1c** Auto-Hare on `opened` / `synchronize`. **Interim (now):** orchestrator posts `<!-- searchts-r1-needed -->`; human/hourly spawns a local cheap-scout (any free model via **ocx**, name that id). **When we build:** GitHub Action = doorbell. Brain = **same free Zen / Nous API as cheap-scouts** (key in GitHub secrets, not the repo). Writer ≠ Hare (CI model ≠ orchestrator). Skip never holds. **Not** a paid MiniMax bill at searchts volume. **Not R1b** (badge). **Not F15**. Do not run the orchestrator as Hare.
  - **Failover (local Grok already does this; the Action must too):** fail = 429 / 5xx / timeout / empty body / no `<!-- searchts-r1-review -->` token. **One** retry on a different model from a **small ordered list** in the workflow (not a live bench, not random OpenRouter). Still dead → post `searchts-r1-needed`, do **not** post a fake review, do **not** become Hare. Paid F15 still needs this (quota / 529). **R1c boring:** no LLM in this Action. Optional later (still dumb): skip an id that 429’d in the last hour (a timestamp, not a model). **Does not have to stay on the laptop forever:** a hosted picker (tiny service / extra job that ranks a catalog) is an **F15** thread, after this Action has been boring. Not a second Grok in R1c. Deprecations / “this model is trash this week” = edit the list until that picker exists.
  - **Boring first:** it fires on every push, posts the two surfaces, nags when the list is exhausted. Intent **ship** only if required checks are green or skipped-as-designed (see AGENTS.md). Red `ci / test` = hold. No landing page, no `hare[bot]`. Done when a week of PRs get a review without anyone pasting a prompt.
...
- [x] **F12** Wall playbook (docs): **P3.11** → **F1** (shipped #145) → **`--human` / F7** (extras only, next). Never **N1** / **N3** / **N7**. README unlocker section. Not a Reddit-green 0.8.1.
...
- [ ] **F15** Hare as a **review product** (other repos install an App, billed, CodeRabbit-shaped). **Different intent from searchts.** Own thread, own glossary: Model = provider + API model, not ocx / cheap-scout / this chat. R1d refresh, Checks-before-Intent (Check Runs API; never ship a red required job), **and** primary+fallback providers (same fail rule as R1c; paid still 529s). Do not mix sprints or tokens with the unlocker. **Revisit:** only as its own thread, after R1c has been boring.
...
| 2026-09-09 | **R1c failover:** one retry on an ordered model list; then `searchts-r1-needed`. Not a live bench. F15 inherits it. |
