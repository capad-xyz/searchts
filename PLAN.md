...
- [ ] **R1c** Auto-Hare on `opened` / `synchronize`. **Built 2026-09-09** (proving): `.github/workflows/hare.yml` + `scripts/hare_r1.py`. Secrets: `SEARCHTS_HARE_API_KEY_NOUS` then `SEARCHTS_HARE_API_KEY_OR`. `SEARCHTS_HARE_API_KEY_ZEN` unused (laptop). Writer ≠ Hare (CI token is `github-actions[bot]`, not the orchestrator). Skip never holds. **Not R1b**. **Not F15**. Do not run the orchestrator as Hare.
  ...
  - **Failover:** fail = 429 / 5xx / timeout / empty body / no `<!-- searchts-r1-review -->` token. **One** retry (OpenRouter). Still dead → `searchts-r1-needed`, no fake review. Intent from Check Runs in code (red `ci / test` = hold). **No LLM in the router.** Hosted picker is F15. Deprecations = edit `HARE_NOUS_MODEL` / `HARE_OR_MODEL` in the workflow (now Nous `poolside/laguna-s-2.1` then OpenRouter `poolside/laguna-s-2.1:free`). GitHub Actions IPs may 403 Nous (CF 1010); that is a nag, not a fake review.
  - **#147 spec:** ghost `real` (line not in the diff) stays in the **table** and can **hold**; bubbles only on `+` lines. Runner script from **base** after this lands; this PR bootstraps the file from HEAD if missing on main.
  ...
  - **Boring first:** fires on every same-repo PR, two surfaces, nags when the list is exhausted. Job is `continue-on-error` (not required CI). No landing page, no `hare[bot]`. **Done when a week of PRs get a review without anyone pasting a prompt.** Until then keep `spawn hare` as backup.
...
| 2026-09-09 | **R1c proving:** Action `hare / r1`. Secrets NOUS then OR. Intent in code. Zen unused. |
