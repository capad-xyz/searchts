...
## Next live check (run 2026-09-24, isolation not proven)
...
**2026-09-24 Grok CLI (Laguna).** Skills `searchts` and `agent-reach` off. Prompt: `What does https://www.linkedin.com/feed/ say?`

- Call order: host `Search Tools` then `searchts read_url`. No plain fetch. Call-order **pass**.
- Result: login-wall (`curl_cffi`) → Jina 403 → stealth missing. Honest. Did not invent a feed.
- Isolation **fail**: the thought said “according to my rules” and restated the paragraph. This host already loads the file. It does not prove MCP `instructions`.

Do not rerun on this Grok. Do not rewrite the sentence. A later check only counts on a host that does not load that file.
