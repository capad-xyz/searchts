...
def test_transcribe_removes_temp_dir_on_success(monkeypatch):
    ...
    def fake_run(source, work_dir, provider, cfg, prefer_subtitles, progress=False, **_kw):
...
def test_transcribe_removes_temp_dir_on_failure(monkeypatch):
    ...
    def boom(source, work_dir, provider, cfg, prefer_subtitles, progress=False, **_kw):
        captured["dir"] = work_dir
