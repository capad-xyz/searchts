...
from searchts.backends import opencli_status, opencli_summary
from searchts.probe import ProbeResult
...
def _status_with(version_probe, daemon_probe=None, ext_on_disk=False):
    ...
    with patch("searchts.backends.opencli.probe_command", side_effect=fake_probe), \
         patch(
             "searchts.backends.opencli._extension_installed_on_disk",
...
def test_not_installed():
    ...
    assert not st.ready
    assert "is not installed" in opencli_summary(st)
...
def test_sleeping_extension_counts_as_ready():
    """Verified live: when the extension service worker is sleeping, daemon status reports disconnected,
    but any real command wakes it -- being installed on disk counts as available."""
    daemon_out = "Daemon: running (PID 1)\nExtension: disconnected\n"
    st, _ = _status_with(
        ProbeResult("ok", output="1.8.3"),
        ProbeResult("ok", output=daemon_out),
    ...
    assert st.ready
    assert "wakes automatically" in opencli_summary(st)
    assert st.hint == ""
